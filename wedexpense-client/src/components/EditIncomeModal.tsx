...
const EditIncomeModal: React.FC<Props> = ({ income, isOpen, onClose, onSaved }) => {
  const [paymentStatus, setPaymentStatus] = useState(income?.payment_status || 'Pending');
  const [amountReceived, setAmountReceived] = useState(income?.amount_received?.toString() || '0');
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      setError('');
      await updateIncome(income.ROWID, {
        ...
        >
          ...
          >
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-5">
              <p className="text-sm font-medium text-white">{income?.source_name || income?.description}</p>
              <p className="text-xs text-white/50 mt-1">Total: {formatINR(parseFloat(income?.amount || 0))}</p>
            </div>
            ...
            <div className="mb-5">
              ...
              <div className="flex flex-wrap items-center gap-3">
                {['Received', 'Pending', 'Partial'].map((opt) => (
                  <label
                    key={opt}
