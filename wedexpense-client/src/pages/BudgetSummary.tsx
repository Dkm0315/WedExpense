...
const BudgetSummary: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [weddingData, summaryData, eventSummary, categorySummary] =
        await Promise.all([
          getWedding(wid),
          getWeddingSummary(wid).catch(() => null),
          getEventSummary(wid).catch(() => []),
          getCategorySummary(wid).catch(() => []),
        ]);
  ...
  const totalSpent = parseFloat(
    summary?.total_spent || wedding?.total_spent || '0'
  );
  const remaining = totalBudget - totalSpent;

  // Build event chart data
  ...
  // Build bride vs groom split from per_side array
  const perSide = Array.isArray(summary?.per_side) ? summary.per_side : [];
  const sideMap: Record<string, number> = {};
  perSide.forEach((s: any) => {
    sideMap[s.paid_by] = parseFloat(s.total || '0');
  });
  const sideChartData = [
      ...
      >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <motion.button
                ...
                onClick={async () => {
                  ...
                  setAiInsights('');
                  try {
                    const result = await getAIInsights(wid);
              ...
              <div className="space-y-4">
                {aiDataSummary && (
                  <div className="flex flex-wrap gap-3 text-xs">
                    <span className="px-3 py-1 bg-white/5 rounded-full text-white/50">
                      {aiDataSummary.expense_count} expenses analyzed
                    </span>
                    <span className="px-3 py-1 bg-white/5 rounded-full text-white/50">
                      {aiDataSummary.event_count} events
                    </span>
                    <span className="px-3 py-1 bg-white/5 rounded-full text-white/50">
                      {aiDataSummary.percent_used}% budget used
                    </span>
                  </div>
                )}
            ...
            {!aiLoading && !aiInsights && (
              <p className="text-white/30 text-sm">
                Click "Ask AI for Insights" to get smart budget analysis powered by AI.
              </p>
            )}
          </motion.div>
        )}
        ...
        {eventChartData.length === 0 && categoryChartData.length === 0 && (
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-12 text-center">
            ...
            <p className="text-white/30 text-sm">
              Start adding expenses to see budget breakdowns and charts.
            </p>
          </div>
        )}
      </motion.div>
